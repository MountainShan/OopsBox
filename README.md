...
| Create Project | SSH Remote |
|:---:|:---:|
| ![Create Project](docs/screenshots/local_create.png) | ![SSH Remote](docs/screenshots/ssh.png) |
...
Channels let your Claude Code instance become a Telegram bot. You chat with it on Telegram, it codes on your server. From the dashboard, click `+ Channel`, paste your BotFather token (stored encrypted, you're welcome), and start it. Claude auto-configures the Telegram plugin, you pair with a code, and boom — you're coding from Telegram on a bus.
...
Channel 讓你的 Claude Code 變成 Telegram bot。你在 Telegram 上聊天，它在 server 上寫 code。從 dashboard 按 `+ Channel`，貼上 BotFather 的 token（加密儲存，不客氣），啟動。Claude 會自動設定 Telegram plugin，你用配對碼連上，然後 — 你就在公車上用 Telegram 寫 code 了。
