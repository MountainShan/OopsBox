...
- **Project isolation** — each project can optionally run its AI agent inside a Podman/Docker container with resource limits. because letting an AI agent have full access to your server is fine until it isn't. configurable per-project with `--memory` and `--cpus` limits, bind-mounts only the project directory, and the agent can't see your other projects. it's the responsible thing to do. I should have done this from the start. I did not.
...
**Local** — agent runs on the box, has full access, does its thing. optionally runs inside an isolated container if you're the responsible type (see below).

**Local (Isolated)** — same as local, but the AI agent runs inside a Podman/Docker container. it can only see its own project directory, has memory and CPU limits, and can't accidentally `rm -rf` your other projects. enable it when creating a project with `isolated: true`. you'll need Podman or Docker installed. the agent image builds automatically on first use.
...
- **專案隔離** — 每個專案可以選擇讓 AI agent 跑在 Podman/Docker container 裡，有資源限制。因為讓 AI agent 完整存取你的 server 在它搞砸之前都沒問題。每個專案可以設 `--memory` 和 `--cpus` 限制，只 bind mount 專案目錄，agent 看不到你其他的專案。這是負責任的做法。我應該一開始就做的。我沒有。
...
**本機** — agent 在這台機器上跑，有完整存取權。如果你是負責任的人，可以選擇跑在隔離 container 裡（見下面）。

**本機（隔離）** — 跟本機一樣，但 AI agent 跑在 Podman/Docker container 裡。它只看得到自己的專案目錄，有記憶體和 CPU 限制，不會不小心 `rm -rf` 你的其他專案。建立專案時用 `isolated: true` 啟用。需要裝 Podman 或 Docker。Agent image 第一次用的時候會自動 build。
